...
architecture: "See diagram below (screenshots/dreamhouse237-architecture.png) — Gateway → Eureka → services, Config Server feeding shared config to all of them"
...
Here's the shape of it end to end — the gateway sits in front, Eureka keeps track of who's alive, and the config server quietly feeds everyone the same shared settings:

![DreamHouse-237 service discovery architecture](screenshots/dreamhouse237-architecture.png)
